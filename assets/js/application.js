...
(function ($) {
  ...
  var cassetteXHR = $.getJSON(
    app.settings.cassettePath,
    function (json) {},
  ...
  cassetteXHR.done(function (json) {
    ...
    var binding = Bind(
      {
        pages: app.cassette.hasOwnProperty("pages")
          ? Object.keys(app.cassette.pages)
          : "",
        contentHTML: "",
      },
      {
        pages: {
          ...
          transform: function (value) {
            ...
            if (
              app.cassette.pages[value].hasOwnProperty("type") &&
              app.cassette.pages[value].type == "external"
            ) {
              variables.pageURL = app.cassette.pages[value].location;
              output = micromustache.render(
                app.templates.externalMenuItem,
                variables,
              );
            } else {
    ...
    var releaseMarkup = function (params) {
      ...
      var jqxhr = $.getJSON(release.playlist, function () {}).done(
        function (json) {
          var route = params.title;
          if (params.artist) {
            route = params.artist + "/" + params.title;
          ...
          */
          if (playlist.hasOwnProperty("title") && playlist.title.length > 0) {
            markup.title =
              '<h1 class="title">' +
              app.templates.playAll +
              ' <a class="navigo-delegate" href="' +
              releaseURL +
              '">' +
              playlist.title +
              creator +
          ...
          if (release.hasOwnProperty("artwork") && release.artwork.length > 0) {
            markup.artwork +=
              '<div class="artwork"><img src="' + release.artwork + '"/></div>';
          }
          if (playlist.hasOwnProperty("track") && playlist.track.length > 0) {
            ...
            $.each(playlist.track, function (index, value) {
              ...
              if (
                params.hasOwnProperty("trackNumber") &&
                params.trackNumber == trackId
              ) {
                markup.tracklist +=
                  '<li class="highlight"><a data-pushtape-permalink = "release/' +
                  route +
                  "/track/" +
                  trackId +
                  '" data-pushtape-sound-id ="' +
                  params.title +
                  "_" +
                  trackId +
                  '" href="' +
                ...
                markup.tracklist +=
                  '<li><a data-pushtape-permalink = "release/' +
                  route +
                  "/track/" +
                  trackId +
                  '" data-pushtape-sound-id ="' +
          ...
          binding.contentHTML =
            markup.title + markup.artwork + markup.tracklist;
          if (release.hasOwnProperty("notes")) {
            ...
            if (
              release.notes.hasOwnProperty("format") &&
              release.notes.format == "json"
            ) {
              var jqxhr = $.getJSON(release.notes.location).done(
                function (data) {
              ...
              var jqxhr = $.get(release.notes).done(function (data) {
    ...
    var allReleasesMarkup = function (releaseTitle) {
      ...
      markup.artwork +=
        '<div class="artwork thumbnail"><img src="' +
        release.artwork +
        '"/></div>';
      return (
        '<a class="release navigo-delegate" href="' +
        releaseURL +
        '">' +
        markup.artwork +
        ...
        if (value.hasOwnProperty("format") && value.format == "json") {
          var jqxhr = $.getJSON(value.location)
            ...
            .fail(function (data, status) {
              binding.contentHTML =
                "<h1>Error</h1> Problem loading page. (" + status + ")";
            })
            .always(function () {});
        } else {
    ...
    pages["releases"] = function (params) {
      if (
        app.cassette.hasOwnProperty("releases") &&
        Object.keys(app.cassette.releases).length > 0
      ) {
